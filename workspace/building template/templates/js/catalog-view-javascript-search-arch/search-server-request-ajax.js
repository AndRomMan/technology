...
// #SHOP search-server-request-ajax.js
(function () {
  ...
  function getSuggestList(value) {
    ...
    $.ajax({
      // BM Проверка поиска заголовков товаров через Get запрос
  ...
  function sendSearchInput(value) {
    redirectToSearchResult(value);

    // BM: получение данных по AJAX
    // localStorage.setItem(SEARCH_VALUE_KEY, value);
    // ajaxRequest(value);
  }

  // function ajaxRequest(value) {
  // BM: Проверка поиска товаров через Get запрос
  ...
  //     dataType: 'html',
  //     success(response) {
  //       window.searchGetResult.pageRendering(response, value);
  //     },
  //     error(jqXHR, textStatus, errorThrown) {
  //       showErrorStatus(jqXHR, textStatus, errorThrown);
  //     },
